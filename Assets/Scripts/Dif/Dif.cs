...
public class Dif : MonoBehaviour
{
    ...
    public bool GenerateMesh(int interiorIndex)
    {
        ...
        // Load DIF resource
        var resource = DifResourceManager.getResource(
        ...
        for (int mat = 0; mat < resource.triangleIndices.Length; mat++)
        {
            ...
            string matName = Path.GetFileNameWithoutExtension(
                resource.materials[mat]
            );
        ...
        foreach (var kvp in materialTriMap)
        {
            ...
            {
                int triCount = Mathf.Min(
                    maxTrianglesPerChunk * 3,
                    allTris.Count - i
                );
    ...
    Material ResolveMaterial(string materialName)
    {
        ...
        {
            Texture2D tex = LoadTexture(texPath, false);
        ...
        string normalPath = ResolveTexturePath(
            materialName,
            ".normal"
        );
        ...
        {
            Texture2D normalTex = LoadTexture(
                normalPath,
                true
            );
    ...
    private Mesh FlatShadePhysicsMesh(
        Mesh mesh
    )
    {
        var oldVerts = mesh.vertices;
    ...
    static Dictionary<
        string,
        (float friction, float restitution, float force)
    > materialDict =
        new Dictionary<
            string,
            (float, float, float)
        >()
        {
            ...
            { "mmg_ice_shadow", (0.2f, 0.95f, 0f) },

            // Multiplayer
            { "friction_mp_high", (6.0f, 0.3f, 0f) },
            { "friction_mp_high_shadow", (6.0f, 0.3f, 0f) },

            // PlatinumQuest
            { "pq_lava", (1.0f, 2.0f, 15f) },
            { "pq_friction_space", (0.01f, 0.35f, 0f) },
            { "pq_friction_ice", (0.07331f, 0.75f, 0f) },
            { "pq_friction_ice_with_danger", (0.07331f, 0.75f, 0f) },
            { "pq_friction_mud", (0.30f, 0.5f, 0f) },
            { "pq_friction_grass", (2.0f, 0.5f, 0f) },
            { "pq_friction_sand", (4.0f, 0.15f, 0f) },
            { "pq_friction_bouncy", (0.2f, 0.0f, 15f) },
            { "pq_friction_water", (6.0f, 0.0f, 0f) },
            { "base.sandhill", (4.0f, 0.15f, 0f) },

            // MBG / old materials
            { "bumper-rubber", (0.5f, 0.0f, 15f) },
            { "triang-side", (0.5f, 0.0f, 15f) },
            { "triang-top", (0.5f, 0.0f, 15f) },
            { "pball-round-side", (0.5f, 0.0f, 15f) },
            { "pball-round-top", (0.5f, 0.0f, 15f) },
            { "pball-round-bottm", (0.5f, 0.0f, 15f) },
            { "bumper", (0.5f, 0.0f, 15f) },
            { "sigil_glow", (0.5f, 0.0f, 15f) },

            // Buttons
            { "base.button", (1.0f, 1.0f, 0f) },
            { "black.button", (1.0f, 1.0f, 0f) },
            { "blue.button", (1.0f, 1.0f, 0f) },
            { "gray.button", (1.0f, 1.0f, 0f) },
            { "green.button", (1.0f, 1.0f, 0f) },
            { "magenta.button", (1.0f, 1.0f, 0f) },
            { "mint.button", (1.0f, 1.0f, 0f) },
            { "orange.button", (1.0f, 1.0f, 0f) },
            { "pink.button", (1.0f, 1.0f, 0f) },
            { "purple.button", (1.0f, 1.0f, 0f) },
            { "yellow.button", (1.0f, 1.0f, 0f) },

            // MBG texture materials
            { "grid_warm", (1.0f, 1.0f, 0f) },
            { "grid_cool", (1.0f, 1.0f, 0f) },
            { "grid_neutral", (1.0f, 1.0f, 0f) },
            { "stripe_cool", (1.0f, 1.0f, 0f) },
            { "stripe_neutral", (1.0f, 1.0f, 0f) },
            { "stripe_warm", (1.0f, 1.0f, 0f) },
            { "tube_cool", (1.0f, 1.0f, 0f) },
            { "tube_neutral", (1.0f, 1.0f, 0f) },
            { "tube_warm", (1.0f, 1.0f, 0f) },
            { "solid_cool1", (1.0f, 1.0f, 0f) },
            { "solid_cool2", (1.0f, 1.0f, 0f) },
            { "solid_neutral1", (1.0f, 1.0f, 0f) },
            { "solid_neutral2", (1.0f, 1.0f, 0f) },
            { "solid_warm1", (1.0f, 1.0f, 0f) },
            { "solid_warm2", (1.0f, 1.0f, 0f) },
            { "pattern_cool1", (1.0f, 1.0f, 0f) },
            { "pattern_cool2", (1.0f, 1.0f, 0f) },
            { "pattern_neutral1", (1.0f, 1.0f, 0f) },
            { "pattern_neutral2", (1.0f, 1.0f, 0f) },
            { "pattern_warm1", (1.0f, 1.0f, 0f) },
            { "pattern_warm2", (1.0f, 1.0f, 0f) },
            ...
            // Ice Shard
            { "shard_snow", (0.0f, 0.0f, 0f) },
            { "base.shard_ice", (0.0f, 0.0f, 0f) },

            // PQ Hazards
            { "roundbumper_tex", (0.2f, 0.0f, 15f) },
            { "tribumper_tex", (0.2f, 0.0f, 15f) },

            // PQ Repulsion
            { "repul_stripe_caution", (1.0f, 1.0f, 10f) },
            { "repul_pq_construction_concrete", (1.0f, 1.0f, 10f) },
            { "pq_ray_wall_combo_repul", (1.0f, 1.0f, 5f) },

            // Spooky
            { "spooky_acidwater", (6.0f, 0.0f, 0f) },
            { "spooky_dirt", (6.0f, 0.3f, 0f) },
            { "spooky_grass", (2.0f, 0.75f, 0f) },

            // Christmas
            { "XmasIce", (0.03f, 0.95f, 0f) },
            { "XmasIceShadow", (0.03f, 0.95f, 0f) },
            { "XmasSnow", (3.0f, 0.2f, 0f) },
            { "XmasSnowShadow", (3.0f, 0.2f, 0f) },

            { "stickconcrete", (1f, 0f, 0f) },
        };
}