...
public class ScreenSpaceReflectionURP : ScriptableRendererFeature
{
    ...
    [Tooltip(
        "Enable this to execute SSR in Rendering Debugger view. This is disabled by default to avoid affecting the individual lighting previews."
    )]
    ...
    [Tooltip(
        "Enable this to denoise SSR at anytime in SceneView. This is disabled by default because URP SceneView only updates motion vectors in play mode."
    )]
    ...
    private static readonly FieldInfo renderingModeFieldInfo = typeof(UniversalRenderer).GetField(
        "m_RenderingMode",
        BindingFlags.NonPublic | BindingFlags.Instance
    );
    private static readonly FieldInfo normalsTextureFieldInfo = typeof(UniversalRenderer).GetField(
        "m_NormalsTexture",
        BindingFlags.NonPublic | BindingFlags.Instance
    );
    ...
    public override void Create()
    {
        ...
        if (material != null)
        {
            if (material.shader != Shader.Find(ssrShaderName))
            {
                Debug.LogErrorFormat(
                    "Screen Space Reflection URP: Material shader should be {0}.",
                    ssrShaderName
                );
                return;
            }
        ...
        if (screenSpaceReflectionPass == null)
        {
            ...
            screenSpaceReflectionPass.renderPassEvent =
    ...
    )
    {
        ...
        var renderingMode = (RenderingMode)
            renderingModeFieldInfo.GetValue(renderer as UniversalRenderer);
        bool isUsingDeferred =
            (renderingMode != RenderingMode.Forward)
            && (renderingMode != RenderingMode.ForwardPlus); // URP may have Deferred+ in the future.
        ...
        )
        {
            ...
            screenSpaceReflectionPass.renderPassEvent =
                ssrVolume.algorithm == ScreenSpaceReflection.Algorithm.PBRAccumulation
                    ? (
                        ssrVolume.accumFactor.value == 0.0f
        ...
        {
            Debug.Log(
                "Screen Space Reflection URP: Disable effect to avoid affecting rendering debugging."
            );
            isLogPrinted = true;
        }
    }
        ...
        public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            if (ssrVolume.algorithm == ScreenSpaceReflection.Algorithm.PBRAccumulation)
            {
                ...
                RenderingUtils.ReAllocateIfNeeded(
                    ...
                    name: "_ScreenSpaceReflectionHitTexture"
                );
                ...
                RenderingUtils.ReAllocateIfNeeded(
                    ...
                    FilterMode.Point,
                    TextureWrapMode.Clamp,
                    name: "_ScreenSpaceReflectionHistoryTexture"
                );
            ...
            else
            {
                ...
                FilterMode filterMode =
                    (mipmapsMode == MipmapsMode.Trilinear)
    ...
    public class BackFaceDepthPass : ScriptableRenderPass
    {
        ...
        {
            if (
                ssrVolume.thicknessMode.value == ScreenSpaceReflection.ThicknessMode.ComputeBackface
            )
            {
                CommandBuffer cmd = CommandBufferPool.Get();
                using (new ProfilingScope(cmd, new ProfilingSampler(profilerTag)))
                {
                    ...
                    RendererListDesc rendererListDesc = new(
                        new ShaderTagId("DepthOnly"),
                        renderingData.cullResults,
                        renderingData.cameraData.camera
                    );
                    depthRenderStateBlock.depthState = new DepthState(
                        true,
    ...
    public class ForwardGBufferPass : ScriptableRenderPass
    {
        ...
        public override void OnCameraSetup(CommandBuffer cmd, ref RenderingData renderingData)
        {
            ...
            if (
                normalsTextureFieldInfo.GetValue(renderingData.cameraData.renderer)
                    is not RTHandle normalsTextureHandle
